...
function OrderReconciliation({ tenantId }) {
  ...
  return <Card className="table-card"><PanelHeader title="Order-wise gross → fees → payout" subtitle={source||'Loading Amazon money lines'} />{error&&<p className="alert error">{error}</p>}{orders.length?<div className="table-wrap"><table><thead><tr>{['Order','Amazon status','Date','Gross','Referral','Fulfillment','Other fees','Refund','Net payout','Reconciliation',''].map(label=><th key={label}>{label}</th>)}</tr></thead><tbody>{orders.map(order=>{const flag=flagByOrder.get(order.amazon_order_id);const detail=details[order.amazon_order_id];return <React.Fragment key={order.amazon_order_id}><tr><td>{order.amazon_order_id}</td><td>{order.status??'—'}</td><td>{String(order.order_date??'').slice(0,10)}</td><td>{Number(order.gross_item_price)?formatCurrency(order.gross_item_price):'Items pending'}</td><td>{order.hasFeeData?formatCurrency(order.referral_commission):'—'}</td><td>{order.hasFeeData?formatCurrency(order.fulfillment_fee):'—'}</td><td>{order.hasFeeData?formatCurrency(Number(order.shipping_fee)+Number(order.closing_fee)+Number(order.other_fees)):'—'}</td><td>{order.hasFeeData?formatCurrency(order.refund):'—'}</td><td><b>{order.hasFeeData?formatCurrency(order.net_payout):'—'}</b></td><td>{flag?<span className="overcharge-badge">Overcharged {formatCurrency(flag.variance)}</span>:order.hasFeeData?<span className="pill status-completed">Reconciled</span>:<span className="pill status-idle">Pending settlement</span>}</td><td><Button variant="ghost" onClick={()=>toggle(order.amazon_order_id)}>{openId===order.amazon_order_id?'Hide':'Details'}</Button></td></tr>{openId===order.amazon_order_id&&<tr className="order-detail-row"><td colSpan="11">{detail?<div className="order-detail-grid"><div><h4>Items</h4>{detail.items.length?detail.items.map((item,index)=><p key={index}><b>{item.title}</b><br/>{item.sku} · {formatNumber(item.quantity_ordered)} × {formatCurrency(item.item_price)}{item.package_weight?` · ${item.package_weight} ${item.weight_unit??''}`:''}</p>):<p className="muted">Order items have not been returned by Amazon yet. Run Orders & finance sync again after the order is confirmed.</p>}</div><div><h4>Why the payout changed</h4>{detail.fees.length?detail.fees.map((fee,index)=><div className={Number(fee.amount)<0?'fee-line negative':'fee-line'} key={index}><span>{fee.amount_description||fee.category}</span><small>{fee.category}</small><strong>{formatCurrency(fee.amount)}</strong></div>):<p className="muted">Amazon has not posted itemized fee data for this order yet. Pending/unshipped orders do not have settlement deductions.</p>}</div></div>:<Empty text="Loading order details…" />}</td></tr>}</React.Fragment>})}</tbody></table></div>:<Empty text="No synced orders in this period."/>}</Card>;
}
