...
function AmazonConnectionPanel({ tenantId, seller, onChange, setError }) {
  ...
  async function restore() {
    setBusy(true);
    try {
      await api(`/api/tenants/${tenantId}/amazon/restore`, { method: 'POST' });
      localStorage.removeItem(`${ACCESS_TOKEN_CACHE_PREFIX}${tenantId}`);
      await onChange();
    } catch (e) { setError(e.message); }
    finally { setBusy(false); }
  ...
  return (
    <div className="amazon-chip is-disconnected">
      ...
      <div className="chip-copy"><b>Amazon not connected</b><small>{seller?.canRestore?'Stored permission found — validate it with Amazon':'Authorize Seller Central to enable sync'}</small></div>
      <Button disabled={busy} onClick={seller?.canRestore?restore:connect}>{busy ? 'Checking…' : seller?.canRestore?'Restore Amazon':'Connect Amazon'}</Button>
    </div>
  );
}
