...
function OrderReconciliation({ tenantId }) {
  ...
  const [orderSearch,setOrderSearch]=useState(''); const [ledgerFilters,setLedgerFilters]=useState({account:'',type:'',status:'',orderId:''});
  ...
  const filteredOrders=orders.filter(order=>String(order.amazon_order_id).toLowerCase().includes(orderSearch.trim().toLowerCase()));
