...
export function calculateDashboardMetrics(input,range){
  ...
  const diagnostics={sourcePolicy:{financial:`${financialSource} (${settlementComplete?'complete statement takes precedence':'settlement incomplete; Finances fallback for KPIs only'})`,accountActivity:'Settlement reports only; Finances is never added',reimbursements:financeReimbursements.length?financialSource:'Reimbursements report fallback',gst:'Imported GST B2B/B2C rows only',settled:'Settlement headers filtered by deposit_date'},requestedRange:range,reportCoverage:coverage,coverageComplete,diagnosticGroups,includedRows:financialRows.length,excludedRows:(settlementComplete?financeAudit.included.length:settlementAudit.included.length),duplicateRows:financialDuplicates.length+itemAudit.duplicates.length+returnAudit.duplicates.length+gstAudit.duplicates.length,unmappedRows:unmappedRows.map(row=>({source_row_id:row.source_row_id,source_report_id:row.source_report_id,settlement_id:row.settlement_id,amount_type:row.amount_type,amount_description:row.amount_description,amount:amount(row),reason:row.classification.reason})),unmappedAmount:signedSum(unmappedRows),categoryTotals:{grossSales,productRefunds,netPromotions,expenseDebits,expenseCredits,tcsTds,operationalFees,gst,tax,transfers}};
