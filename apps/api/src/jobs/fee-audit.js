...
export async function runFeeAuditForTenant(tenantId, options = {}) {
  ...
  return runJob(`fee-audit:${parsedTenantId}`, async () => {
    ...
    return withTenant(parsedTenantId, async db => {
      const items = (await db.query(`select oi.amazon_order_id, oi.sku, oi.asin, oi.item_price, oi.quantity_ordered, o.fulfillment_channel
      ...
      for (const item of items) {
        const cacheKey = `${item.sku}:${Number(item.item_price)}`;
        let estimate = estimates.get(cacheKey);
        if (!estimate) estimate = (await db.query(`select * from fee_estimates where tenant_id=$1 and sku=$2 and price_snapshot=$3 and estimated_at > now()-interval '30 days'`, [parsedTenantId, item.sku, item.item_price])).rows[0];
        if (!estimate) {
          const request = { FeesEstimateRequest: { MarketplaceId: seller.marketplace_id, IsAmazonFulfilled: item.fulfillment_channel === 'AFN', PriceToEstimateFees: { ListingPrice: { CurrencyCode: 'INR', Amount: amount(item.item_price) }, Shipping: { CurrencyCode: 'INR', Amount: 0 } }, Identifier: `fee-estimate-${item.sku}-${Date.now()}` } };
          ...
          if (!parsed && item.asin) {
            try {
              const catalog = await spApi.getCatalogItem(item.asin, seller.marketplace_id); const slab = slabForCategory(catalog.category); const weightFee = computeWeightFee(weightKg(catalog.weightNode)); const priceFees = computeSlabFees(slab, amount(item.item_price));
        ...
        orderEstimate.expected += amount(estimate.total_fee) * Math.max(1, Number(item.quantity_ordered ?? 1)); orderEstimate.skus.add(item.sku); orderEstimate.sources.add(estimate.source); orderEstimates.set(item.amazon_order_id, orderEstimate);
      }
      for (const [orderId, estimate] of orderEstimates) {
        const actualRow = (await db.query(`select count(*) count, coalesce(sum(abs(amount)),0) actual from finance_transaction_items where tenant_id=$1 and order_id=$2 and amount<0 and category=any($3)`, [parsedTenantId,orderId,['referral_commission','fulfillment_fee_per_order','fulfillment_fee_per_unit','fulfillment_fee_weight','closing_fee']])).rows[0];
