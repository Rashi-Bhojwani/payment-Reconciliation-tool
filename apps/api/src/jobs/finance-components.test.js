...
test('flattens item breakdowns and resolves array identifiers', () => {
  // Per the official Finances API v2024-06-19 schema, SKU/ASIN live on an
  // Item's `contexts` array as a ProductContext entry - never on a
  // "productDetails"/"productContext" field.
  const rows = flattenFinanceTransaction({ transactionId:'tx-1', postedDate:'2026-01-01T00:00:00Z', relatedIdentifiers:[{relatedIdentifierName:'ORDER_ID',relatedIdentifierValue:'order-1'}], items:[{contexts:[{contextType:'ProductContext',sku:'sku-1',asin:'asin-1'}],breakdown:[{description:'Principal',amount:{currencyAmount:500,currencyCode:'INR'}},{description:'Commission',amount:{currencyAmount:-50,currencyCode:'INR'}}]}] });
...
test('supports 2024 transaction-level plural breakdowns and contexts', () => {
  const rows = flattenFinanceTransaction({ transactionId:'tx-2', relatedIdentifiers:[{relatedIdentifierName:'ORDER_ID',relatedIdentifierValue:'order-2'}], contexts:[{contextType:'ProductContext',sku:'sku-2',asin:'asin-2'}], breakdowns:[{breakdownType:'FBAWeightBasedFee',breakdownAmount:{currencyAmount:-75,currencyCode:'INR'}}] });
