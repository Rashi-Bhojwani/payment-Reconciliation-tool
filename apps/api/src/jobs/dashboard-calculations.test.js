...
test('preserves distinct Amazon rows even when their financial content is identical',()=>{const input=mindcircusFixture();input.settlementRows.push({...input.settlementRows[0],source_row_id:'second-real-line'});input.financeItems.push({transaction_id:'x',category:'summary_amazon_fees',amount:-999,posted_date:'2026-07-10'});const r=calculateDashboardMetrics(input,range);assert.equal(r.metrics.netSales.value,318606.88);assert.equal(r.metrics.reimbursements.value,196.72);assert.equal(r.diagnostics.duplicateRows,0);});
