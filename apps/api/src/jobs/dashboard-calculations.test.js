...
test('uses integer minor units and exposes unknown non-zero labels instead of silently treating them as income',()=>{const input=mindcircusFixture();input.settlementRows.push(line('unknown','Novel Amazon label',99.99,'Novel event',{amount_minor:10001}));const r=calculateDashboardMetrics(input,range);assert.equal(r.statement.income.value,156406.02);assert.equal(r.diagnostics.status,'Does not reconcile');assert.equal(r.diagnostics.unmappedAmount,100.01);assert.equal(r.diagnostics.unmappedRows[0].source_row_id,'unknown');});
