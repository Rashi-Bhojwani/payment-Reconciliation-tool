...
async function loadDashboardCalculations(db, tenantId, range) {
  ...
  const [
    ...
    db.query(
      `select id source_row_id,amazon_order_id,status,fulfillment_channel,order_date,total_amount,raw
       from orders where tenant_id=$1 and order_date >= $2 and order_date < $3`,
      [tenantId, range.start, range.end]
    ),
    db.query(
      `select oi.id source_row_id,oi.source_key,oi.amazon_order_item_id,oi.amazon_order_id,oi.asin,oi.sku,
         oi.title,oi.quantity_ordered,oi.item_price,oi.promotion_discount,oi.raw,o.status,o.fulfillment_channel,o.order_date
       from order_items oi
       join orders o on o.tenant_id=oi.tenant_id and o.amazon_order_id=oi.amazon_order_id
       where oi.tenant_id=$1 and o.order_date >= $2 and o.order_date < $3`,
      [tenantId, range.start, range.end]
    ),
    db.query(
      `select id source_row_id,source_key,order_id,order_item_id,sku,asin,return_date,return_reason,
         disposition,status,amazon_status,quantity,raw
       from returns
       where tenant_id=$1
    ...
    db.query(
      `select id source_row_id,source_key,settlement_id,order_id,transaction_type,amount_type,
         amount_description,amount,currency,posted_date,order_item_code,sku,quantity,raw,
         transaction_type parent_transaction_type
       from settlement_rows
       where tenant_id=$1 and posted_date >= $2 and posted_date < $3`,
      [tenantId, range.start, range.end]
    ),
    db.query(
      `select settlement_id,settlement_start_date,settlement_end_date,deposit_date,total_amount,currency,report_id,raw
       from settlement_statements where tenant_id=$1`,
      [tenantId]
    ),
    db.query(
      `select fi.id source_row_id,fi.source_key,fi.transaction_id,fi.order_id,fi.sku,fi.asin,fi.category,
         fi.account_section,fi.amount_description,fi.breakdown_path,fi.is_summary,fi.amount,fi.currency,
         fi.posted_date,fi.raw,ft.transaction_type parent_transaction_type,
         ft.transaction_status,ft.description transaction_description,ft.account_type
    ...
    db.query(
      `select transaction_id,transaction_type,transaction_status,description,account_type,posted_date,
         total_amount,currency,related_order_id,raw
       from finance_transactions
       where tenant_id=$1 and posted_date >= $2 and posted_date < $3`,
      [tenantId, range.start, range.end]
    ),
    db.query(
      `select source_key,reimbursement_id,order_id,amount,currency,reason,sku,reimbursement_date,raw
       from reimbursements
       where tenant_id=$1
    ...
    db.query(
      `select id source_row_id,source_key,invoice_type,order_id,document_number,line_id,transaction_type,
         sku,asin,quantity,cgst,sgst,utgst,igst,cess,taxable_value,invoice_amount,tax_total,invoice_date,raw
       from gst_invoices
       where tenant_id=$1
    ...
    db.query(
      `select date,units_ordered,units_refunded
    ...
    loadSourceCoverage(db, tenantId, range)
  ]);
  ...
  const calculations = calculateDashboardMetrics({
    ...
    settlementHeaders: settlementStatements.rows,
    ...
    salesTrafficDaily: salesTrafficDaily.rows,
    coverage: sourceCoverage,
    marketplaceTimeZone
  }, range);
  return { ...calculations, sourceCoverage };
}
