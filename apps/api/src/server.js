...
import { getDashboardSync, startDashboardSync } from './jobs/dashboard-sync.js';
...
app.post('/api/tenants/:tenantId/dashboard-sync',async(request,reply)=>{const{tenantId}=TenantParamsSchema.parse(request.params);await requireTenantUser(request,tenantId);await assertActiveTenant(tenantId);const body=DashboardSyncSchema.parse(request.body??{});const seller=(await pool.query("select marketplace_id from sellers where tenant_id=$1 and auth_status='authorized' order by connected_at desc limit 1",[tenantId])).rows[0];if(!seller)throw Object.assign(new Error('Connect an Amazon seller account before syncing dashboard data'),{statusCode:409});if(seller.marketplace_id!==body.marketplaceId)throw Object.assign(new Error('Requested marketplace does not match the connected Amazon account'),{statusCode:409});const marketplace=MARKETPLACES[body.marketplaceId];if(!marketplace)throw Object.assign(new Error('Marketplace timezone is not configured'),{statusCode:400});const endExclusiveDate=new Date(`${body.localEndDate}T00:00:00Z`);endExclusiveDate.setUTCDate(endExclusiveDate.getUTCDate()+1);const localEndExclusive=endExclusiveDate.toISOString().slice(0,10);const job=await startDashboardSync({tenantId,marketplaceId:body.marketplaceId,localStartDate:body.localStartDate,localEndDate:body.localEndDate,range:{start:zonedCalendarInstant(body.localStartDate,marketplace.timeZone),end:zonedCalendarInstant(localEndExclusive,marketplace.timeZone)}});return reply.code(202).send(job);});
app.get('/api/tenants/:tenantId/dashboard-sync/:jobId',async request=>{const{tenantId}=TenantParamsSchema.parse(request.params);const jobId=z.string().uuid().parse(request.params.jobId);await requireTenantUser(request,tenantId);const job=await getDashboardSync(tenantId,jobId);if(!job)throw Object.assign(new Error('Dashboard sync job not found'),{statusCode:404});return job;});
...
async function loadDashboardCalculations(db, tenantId, range, context={}) {
  const [orders,orderItems,returns,settlementRows,settlementHeaders,financeItems,financeTransactions,reimbursements,gstInvoices,settlementReports,settlementControls,failedJobs,completedSources,coverageRows]=await Promise.all([
    ...
    db.query(`select source_row_id,settlement_id,order_id,amount_type,amount_description,amount,amount_minor,currency,posted_date,raw,source_report_id,source_line_number,
      coalesce(transaction_type,raw->>'transaction-type',raw->>'transaction type',raw->>'transactionType') parent_transaction_type
    ...
    db.query(`select settlement_id,deposit_date,header_total_minor::numeric/100 total_amount,transaction_type,raw from settlement_rows where tenant_id=$1 and deposit_date is not null and header_total_minor is not null`,[tenantId]),
    ...
    db.query('select source_type,status,progress,pages_expected,pages_completed,reports_expected,reports_completed,rows_imported,last_successful_sync,error_code,error_message from source_coverage where tenant_id=$1 and marketplace_id=$2 and utc_start=$3 and utc_end=$4',[tenantId,context.marketplaceId,range.start,range.end])
  ]);
  ...
  const completed=new Set(completedSources.rows.map(row=>row.report_type));const coverage=new Map(coverageRows.rows.map(row=>[row.source_type,row]));
  const result=calculateDashboardMetrics({orders:orders.rows,orderItems:orderItems.rows,returns:returns.rows,settlementRows:settlementRows.rows,settlementHeaders:settlementHeaders.rows,settlementControls:settlementControls.rows,financeItems:financeItems.rows,financeTransactions:financeTransactions.rows,reimbursements:reimbursements.rows,gstInvoices:gstInvoices.rows,failedJobs:failedJobs.rows,context:calculationContext,sourceCoverage:coverageRows.rows,sourceCompleteness:{settlementCoverageComplete:coverage.get('settlement_reports')?.status==='complete'&&coverageContains(settlementReports.rows,range),financesCoverageComplete:coverage.get('finances')?.status==='complete',ordersCoverageComplete:coverage.get('orders')?.status==='complete'||completed.has('DIRECT_SP_API_SYNC'),returnsCoverageComplete:coverage.get('returns')?.status==='complete'||completed.has('GET_FBA_FULFILLMENT_CUSTOMER_RETURNS_DATA'),orderItemsCoverageComplete:coverage.get('order_items')?.status==='complete',gstCoverageComplete:coverage.get('gst_b2b')?.status==='complete'&&coverage.get('gst_b2c')?.status==='complete',transferCoverageComplete:coverage.get('transfer_evidence')?.status==='complete'}},range);
  result.diagnostics.reportCoverage=settlementReports.rows;
  result.diagnostics.sourceCoverage=coverageRows.rows;
  return result;
}
