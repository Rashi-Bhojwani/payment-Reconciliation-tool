...
app.post('/api/tenants/:tenantId/reconcile-sources',async request=>{
  ...
  const {range}=z.object({range:DateRangeSchema}).parse(request.body??{});const results=[];
  try{results.push({source:'V2 Settlement reports',status:'completed',...await syncReportForTenant({tenantId,reportType:'GET_V2_SETTLEMENT_REPORT_DATA_FLAT_FILE_V2',range})});}catch(error){results.push({source:'V2 Settlement reports',status:'failed',error:error instanceof Error?error.message:String(error)});}
  try{results.push({source:'Orders and Order Items',status:'completed',...await syncRecentApiDataForTenant(tenantId,{range,includeFinance:false,includeInventory:false,maxOrderPages:100,maxOrderItems:1000,refreshOrderItems:true})});}catch(error){results.push({source:'Orders and Order Items',status:'failed',error:error instanceof Error?error.message:String(error)});}
  return{results};
});
