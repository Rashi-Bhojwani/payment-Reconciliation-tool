...
const SCHEDULED_REPORT_TYPES = new Set(['GET_V2_SETTLEMENT_REPORT_DATA_FLAT_FILE_V2']);
...
export class SpApiClient {
  ...
  async fetchReport(reportType, tenantId, range, marketplaceId = INDIA_MARKETPLACE_ID) {
    ...
    // Settlement V2 is generated by Amazon on its settlement schedule. Calling
    // createReport for it returns 400/FATAL for many accounts; retrieve every
    // completed scheduled document overlapping the requested dashboard range.
    if (SCHEDULED_REPORT_TYPES.has(parsedReportType)) {
      return this.fetchScheduledReports(parsedReportType, parsedRange, marketplaceId);
    }
    ...
    if (!create.ok && create.status === 400 && parsedReportType !== 'GET_V2_SETTLEMENT_REPORT_DATA_FLAT_FILE_V2') {
  ...
  async fetchScheduledReports(reportType, range, marketplaceId = INDIA_MARKETPLACE_ID) {
    const query = new URLSearchParams({
      reportTypes: reportType,
      processingStatuses: 'DONE',
      marketplaceIds: marketplaceId,
      createdSince: range.start,
    ...
    do {
      ...
      const response = await this.request(`/reports/2021-06-30/reports?${query}`);
      if (!response.ok) throw new Error(`List scheduled settlement reports failed: ${response.status}`);
      const body = z.object({ reports: z.array(z.object({
        reportId: z.string(), reportDocumentId: z.string().optional(), processingStatus: z.string(),
        dataStartTime: z.string().optional(), dataEndTime: z.string().optional()
      }).passthrough()).default([]), nextToken: z.string().optional() }).parse(await response.json());
      reports.push(...body.reports.filter(report => report.processingStatus === 'DONE' && report.reportDocumentId));
      nextToken = body.nextToken;
    } while (nextToken);
    if (!reports.length) throw new Error('Amazon has not generated a completed settlement report for this period yet');

    // Amazon can return the same scheduled document on adjacent listing pages.
    const unique = [...new Map(reports.map(report => [report.reportDocumentId, report])).values()]
      .sort((a, b) => String(a.dataStartTime ?? '').localeCompare(String(b.dataStartTime ?? '')));
    const documents = await Promise.all(unique.map(report => this.downloadReportDocument(report.reportDocumentId)));
    const lines = [];
    for (const document of documents) {
      const documentLines = String(document.content).replace(/^\uFEFF/, '').split(/\r?\n/).filter(Boolean);
      if (!documentLines.length) continue;
      if (!lines.length) lines.push(...documentLines);
      else lines.push(...documentLines.slice(1));
    }
    return {
      reportId: unique.map(report => report.reportId).join(','),
      reportDocumentId: unique.map(report => report.reportDocumentId).join(','),
      content: lines.join('\n'),
      compressionAlgorithm: undefined,
      reportsImported: unique.length
    };
  }
